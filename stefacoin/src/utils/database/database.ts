...
const FILTER = {
  ...
  $in: <T>(dado: T, attr: string, value: any[]) => value.includes(dado[attr]),
  $notin: <T>(dado: T, attr: string, value: any[]) => !value.includes(dado[attr])
  //$includes: <T>(dado: T, attr: string, value: number) => dado[attr].includes(value)
};
